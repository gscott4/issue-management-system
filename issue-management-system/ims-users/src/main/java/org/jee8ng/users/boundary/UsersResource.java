...
public class UsersResource {
	...
	public Response add(@ApiParam(value = "User that needs to be added", required = true)
			User newUser, @Context UriInfo uriInfo) {
	...
	public Response authenticate(@ApiParam(value = "user that needs to be authenticated", required = true)
			Credential creds) {
		User validUser =service.isValid(creds.getUsername(), creds.getPassword());
		
		if(validUser != null) {
			String token = issuer.issueToken(creds.getUsername());
			// Set the token and user id as part of response
	...
	public Response update(@ApiParam(value = "ID of user that needs to be updated", required = true)
			@PathParam("id") Long id,
		...
		return done ? Response.ok(updated).build()
	...
	public Response remove(@ApiParam(value = "ID of user that needs to be removed", required = true)
			@PathParam("id") Long id) {
	...
	private URI getLocation(UriInfo uriInfo, Long id) {
